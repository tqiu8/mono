...
namespace DebuggerTests
{
	...
	public class ThreadedSourceList : DebuggerTestBase {
		...
		void CheckNumber (JToken locals, string name, int value) {
			Console.WriteLine($"CHECK NUMBERS {name} {value}");
		...
		async Task<Result> SetBreakpoint (string url_key, int line, int column, DebugTestContext ctx, bool expect_ok=true)
		{
			var bp1_req = JObject.FromObject(new {
				...
				// url = "dotnet://threaded-debugger-test.dll/C:\\Users\\taqiu\\Desktop\\mono\\sdks\\wasm\\threaded-debugger-test.cs"
			});
		...
		public async Task CreateBreakpointAndHit () {
			...
			await insp.Ready (async (cli, token) => {
				...
				await SetBreakpoint ("dotnet://threaded-debugger-test.dll/threaded-debugger-test.cs", 5, 2, ctx);

				var eval_req = JObject.FromObject(new {
					expression = "window.setTimeout(function() { sleep_test(); }, 1);",
				});

				await EvaluateAndCheck (
					"window.setTimeout(function() { sleep_test(); }, 1);",
					"dotnet://threaded-debugger-test.dll/threaded-debugger-test.cs", 5, 2,
					"SleepTest", ctx,
					wait_for_event_fn: (pause_location) => {
						...
						Assert.Equal ("SleepTest", scope["name"]);
		...
		public async Task InspectLocalsDuringSteppingWithSleep () {
		   ...
		   await insp.Ready (async (cli, token) => {
			...
			//    await SetBreakpoint (debugger_test_loc, 25, 12, ctx);
			//    await SetBreakpoint (debugger_test_loc, 32, 12, ctx);
			   await SetBreakpoint (debugger_test_loc, 44, 8, ctx);

			   await EvaluateAndCheck (
				   "window.setTimeout(function() { invoke_add(); }, 1);",
				   debugger_test_loc, 44, 8, "IntAdd", ctx,
				   locals_fn: (locals) => {
					   CheckNumber (locals, "c", 30);
				   }
				);

				// await StepAndCheck (StepKind.Over, debugger_test_loc, 25, 2, "IntAdd", ctx,
				//     locals_fn: (locals) => {
